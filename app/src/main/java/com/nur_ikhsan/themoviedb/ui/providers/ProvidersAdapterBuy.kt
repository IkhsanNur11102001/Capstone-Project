...
import com.nur_ikhsan.themoviedb.data.response.BuyItem
...
class ProvidersAdapterBuy(private val providers : ProvidersUS) : RecyclerView.Adapter<ProvidersAdapterBuy.ViewHolder>() {
    ...
    inner class ViewHolder(private val binding : ItemProvidersDetailBinding) : RecyclerView.ViewHolder(binding.root) {
        fun setProviders(provider: BuyItem) {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val provider = providers.uS.buy[position]
        holder.setProviders(provider)
    }

    override fun getItemCount(): Int = providers.uS.buy.size
}